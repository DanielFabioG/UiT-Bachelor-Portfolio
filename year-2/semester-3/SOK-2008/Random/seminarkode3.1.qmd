...
figur_1 <- axes_1 +
  geom_segment(aes(x = 0, y = Demand(0), xend = 500, yend = Demand(500))) +
    annotate("text", x = 505, y = 10, label = expression(L^D)) +
  geom_segment(aes(x = 0, y = Supply(0), xend = 500, yend = Supply(500))) +
    annotate("text", x = 505, y = 250, label = expression(L^S)) +
  geom_segment(aes(x = L_star, y = 0, xend = L_star, yend = w_star), linetype="dashed", color="red") +
    annotate("text", x = L_star, y = 10, label = expression(L^FK)) +
  geom_segment(aes(x = 0, y = w_star, xend = L_star, yend = w_star), linetype="dashed", color="red") +
    annotate("text", x = L_star + 10, y = w_star, label = expression(w^FK))
...
```{r}
L_S_minW <- 0  # Labor supplied at minimum wage
L_D_minW <- 200  # Labor demanded at minimum wage

figur_2 <- figur_1 +
  geom_segment(aes(x=0, y=w_min, xend=450, yend=w_min)) +
    annotate("text", x=480, y=w_min, label=expression(w^min)) +
  geom_segment(aes(x=L_S_minW, y=0, xend=L_S_minW, yend=w_min), linetype="dashed", color="red") +
    annotate("text", x=L_S_minW, y=10, label=expression(L^{S[minW]})) +
  geom_segment(aes(x=L_D_minW, y=0, xend=L_D_minW, yend=w_min), linetype="dashed", color="red") +
    annotate("text", x=L_D_minW, y=10, label=expression(L^{D[minW]})) +
  geom_polygon(data = data.frame(x = c(L_star, L_D_minW, L_S_minW), 
                                 y = c(w_star, w_min, w_min)),
               aes(x = x, y = y),
               fill = "blue", alpha = 0.3) +
  annotate("text", x=L_star, y=w_star, label="A") +
  annotate("text", x=L_D_minW, y=w_min, label="B") +
  annotate("text", x=L_S_minW, y=w_min, label="C")
