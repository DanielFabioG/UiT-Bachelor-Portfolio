...
```{r}
# Alle unike verdier blir telt ved å bruke binwidth 0.1, og grafen blir mer leselig med observasjonene. Bruker man ikke binwidth eller en veldig høy verdi blir figuren veldig vanskelig å tyde.
data(mtcars)
mtcars %>%
  ggplot(aes(mpg)) +
...
```{r}
# Skifter navn på variabler 
df1<-df %>%
  rename(var = statistikkvariabel) %>%
   rename(workers = sysselsettingsgruppe) %>%
   rename(næring = `næring (SN2007)` )

```

```{r}
# Filtrerer ut data som jeg vil presentere i figuren
df1 %>%
filter(workers %in% c("20-49 sysselsatte", "50-99 sysselsatte", "100-199 sysselsatte", "200-499 sysselsatte", "500 sysselsatte og over")) %>%
  filter(år >= 2015) %>%
  filter(næring %in% c("Industri", "Tjenesteyting", "Andre næringer")) %>%
  
# Plotter figuren, deler verdiene på 1000 for å få de i milliarder
# Bruker histogram for å presentere dataen, og facet_grid for å få det i flere figurer
  ggplot(aes(value/1000, fill = næring)) +
  geom_histogram(binwidth = 0.4) +
  facet_grid(~ workers) +
  theme(legend.position = "bottom") +
  labs(title = "FoU-kostnader fordelt på type næring og ansatte", y = "Antall observasjoner", x = "Verdier i milliarder" )
...
# Plotter figuren, deler verdiene på 1000 for å få de i milliarder
# Bruker histogram for å presentere dataen, og facet_grid for flere figurer
  ggplot(aes(value/1000, fill = var)) +
  geom_histogram(binwidth = 0.2) +
  facet_grid(~ næring) +
  theme(legend.position = "bottom") +
  labs(title = "FoU-kostnader fordelt på type næring", y = "Antall observasjoner", x = "Verdier i milliarder" )
